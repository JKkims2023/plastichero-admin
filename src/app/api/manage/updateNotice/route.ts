...
export async function POST(req: NextRequest) {
    try {
        ...
        // 이미지 파일 처리
        for (let i = 1; i <= 4; i++) {  // 5개에서 4개로 변경 (5번째는 PDF 전용)
        ...
        if (pdfFile && typeof pdfFile !== 'string') {
            console.log('PDF 처리 시작:', (pdfFile as File).name);
            const originalFileName = (pdfFile as File).name;
            const fileName = `pdf-${Date.now()}${path.extname(originalFileName)}`;
            
            // 파일 버퍼로 변환
            const bytes = await pdfFile.arrayBuffer();
            const buffer = Buffer.from(bytes);
            
            // S3에 직접 업로드 - PDF용 컨텐츠 타입과 헤더 설정
            ...
            if (uploadResult.success) {
                console.log('PDF 업로드 성공:', uploadResult.key);
                console.log('PDF URL:', uploadResult.url);
                // S3에 업로드된 파일의 전체 URL을 DB에 저장 (notice_picture5 필드 사용)
                fileData.notice_picture5 = uploadResult.url;
            } else {
                console.error('PDF 업로드 실패:', uploadResult.error);
                uploadErrors.push(`PDF 업로드 실패: ${uploadResult.error}`);
            }
        }
        ...
        console.log('이미지/비디오/PDF 필드 업데이트:', fileData);
