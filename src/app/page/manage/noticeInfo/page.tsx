...
export default function Home() {
    ...
    const [expandedRows, setExpandedRows] = React.useState<Array<string | number>>([]);
    // 미디어 섹션 접기/펼치기 상태 관리
    ...
    const rowsPerPage = 5;
    const [commentFilter, setCommentFilter] = React.useState('all'); // 'all', 'approved', 'unapproved'
        ...
        renderCell: (params) => (
            <Button
                ...
                onClick={(event) => {
                    ...
                    get_CommentInfo(filterNoticeList[params.row.id - 1].notice_no);
                }}
            >
                보기
            </Button>
    ...
    const get_CommentInfo = async(notice_no) => {
        setLoading(true);
        try {
            const response = await fetch('/api/manage/noticeInfo/commentInfo', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ notice_no }),
            });
            ...
            if (response.ok) {
                setCommentList(data.result_data.map((data, idx) => ({ id: idx + 1, ...data })));
            } else {
                alert(data.message);
            }
        } catch (error) {
    ...
    const handleChangeFilterContentType = (event: SelectChangeEvent<number>) => {
        try {
            ...
            switch(event.target.value) {
                ...
                case 20: {
                    setFilterContentTypeValueMethod('title');
                } break;
                case 30: {
                    setFilterContentTypeValueMethod('description');
                } break;
                default: { 
                    setFilterContentTypeValueMethod('all');
                } break;  
    ...
    const handleChangeFilterType = (event: SelectChangeEvent<number>) => {
        try {
            ...
            switch(event.target.value) {
                ...
                case 20: {
                    setFilterStatusValueMethod('Y');
                } break;
                case 30: {
                    setFilterStatusValueMethod('N');
                } break;
                default: {
                    setFilterStatusValueMethod('all');
                } break;
        ...
        try {
            ...
            if(filterStatusValueMethod === 'all') {
                ...
                if(filterInfo.length > 0) {
                    switch(filterContentTypeValueMethod) {
                        case 'title': {
                            filteredList = filteredList.filter((notice) => 
                                notice.notice_title?.toString().toLowerCase().includes(filterInfo.toLowerCase())
                            ).map((notice, idx) => ({ ...notice, id: idx + 1 }));
                        } break;
                        case 'description': {
                            filteredList = filteredList.filter((notice) => 
                                notice.notice_desctription?.toString().toLowerCase().includes(filterInfo.toLowerCase())
                            ).map((notice, idx) => ({ ...notice, id: idx + 1 }));
                        } break;
                        default: {
                            // 모든 필드 검색
                    ...
                    filteredList = filteredList.map((notice, idx) => ({ ...notice, id: idx + 1 }));
                }
            } else {
                // comment_flag로 필터링
                if(filterInfo.length > 0) {
                    switch(filterContentTypeValueMethod) {
                        case 'title': {
                            filteredList = filteredList.filter((notice) => 
                                notice.notice_title?.toString().toLowerCase().includes(filterInfo.toLowerCase()) &&
                                notice.comment_flag === filterStatusValueMethod
                            ).map((notice, idx) => ({ ...notice, id: idx + 1 }));
                        } break;
                        case 'description': {
                            filteredList = filteredList.filter((notice) => 
    ...
    const handleCommentApproval = async (commentId: string, approved: boolean) => {
        try {
            setLoading(true);
            const response = await fetch('/api/manage/noticeInfo/updateComment', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (response.ok) {
                // 댓글 목록 다시 불러오기
                // @ts-ignore
                get_CommentInfo(selectedContent.notice_no);
                
                alert(approved ? '댓글이 승인되었습니다.' : '댓글이 미승인 처리되었습니다.');
            } else {
    ...
    const handleCommentDelete = async (commentId: string) => {
        ...
        try {
            setLoading(true);
            const response = await fetch('/api/manage/noticeInfo/deleteComment', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    // 댓글 목록을 위한 커스텀 행 렌더링 컴포넌트
    const CustomCommentRow = React.memo(function CustomCommentRow(props: any) {
        const { row } = props;
        const isExpanded = expandedRows.includes(row.id);
        const isLongComment = row.comment && row.comment.length > 100;
        ...
        return (
            <Box sx={{ 
                ...
                backgroundColor: row.id % 2 === 0 ? '#fafafa' : 'white',
                height: 'auto',
                boxSizing: 'border-box'
            }}>
                {/* 첫 번째 줄: 번호, 작성자, 상태, 등록일, 관리 버튼 */}
                <Box sx={{ 
                    display: 'flex', 
                    alignItems: 'center', 
                    ...
                    <Box sx={{ 
                        ...
                        <Typography sx={{ width: 60, fontWeight: 500, fontSize: '13px' }}>
                            No. {row.id}
                        </Typography>
                        ...
                        <Box sx={{ 
                            ...
                            alignItems: 'center', 
                            gap: 0.5,
                            color: row.approval_yn === 'Y' ? green[700] : red[700],
                            fontWeight: 500,
                            fontSize: '13px',
                            width: 80
                        }}>
                    ...
                    <Box sx={{ display: 'flex', gap: 1, flexShrink: 0 }}>
                        {row.approval_yn === 'Y' ? (
                            <Button
                                ...
                                color="error"
                                size="small"
                                onClick={() => handleCommentApproval(row.comment_no, false)}
                                sx={{ 
                                    fontSize: '11px',
                                    height: '26px',
                                    minWidth: '60px'
                            ...
                            <Button
                                ...
                                color="success"
                                size="small"
                                onClick={() => handleCommentApproval(row.comment_no, true)}
                                sx={{ 
                                    fontSize: '11px',
                                    height: '26px',
                                    minWidth: '60px'
                        ...
                        <Button
                            ...
                            color="error"
                            size="small"
                            onClick={() => handleCommentDelete(row.comment_no)}
                            sx={{ 
                                fontSize: '11px',
                                height: '26px',
                                minWidth: '60px'
          ...
          alignItems:'center',
            ...
            <div style={{display:"flex", float:"left"}}>
              ...
              <FormControl fullWidth  style={{ width:"175px",marginTop:"0px", marginLeft:"8px", backgroundColor:'white', color:'black'}}>
                  ...
                  <MenuItem style={{fontSize:13}} value={20}>댓글허용</MenuItem>
                  <MenuItem style={{fontSize:13}} value={30}>댓글비허용</MenuItem>
                  </Select>
              </FormControl>
            ...
            <div style={{display:"flex", float:"left", marginLeft:"auto", width:"100%"}}>
              ...
              <FormControl fullWidth sx={{ 
                  ...
                  >
                      ...
                      <MenuItem style={{fontSize:14}} value={20}>공지제목</MenuItem>
                      <MenuItem style={{fontSize:14}} value={30}>공지내용</MenuItem>
                  </Select>
              </FormControl>
            </div>
                    ...
                    <Box sx={{ 
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.2, mb: 2 }}>
                            ...
                            <Box sx={{ display: 'flex', ml: 'auto', alignItems: 'center', gap: 1 }}>
                                <FilterAltIcon sx={{ color: '#666', fontSize: 18 }} />
                                <Box sx={{ 
                                    display: 'flex',
                                    gap: 1
                                }}>
                                    ...
                                    <Chip
                                        ...
                                        size="small"
                                        onClick={() => {
                                            setCommentFilter('unapproved');
                                            setPage(0);
                                        }}
                                        color={commentFilter === 'unapproved' ? 'error' : 'default'}
                                        sx={{ 
                                            fontSize: '12px',
                                            height: '24px'
                                        }}
                            ...
                            <Box sx={{ 
                                ...
                                {(() => {
                                    ...
                                    const filteredComments = commentList.filter(comment => {
                                        ...
                                        if (commentFilter === 'approved') return comment.approval_yn === 'Y';
                                        if (commentFilter === 'unapproved') return comment.approval_yn !== 'Y';
                                        return true;
                                    });
                                    ...
                                    if (filteredComments.length > 0) {
                                        return (
                                            <>
                                                ...
                                                {filteredComments
                                                    ...
                                                    .map((comment) => {
                                                        const isExpanded = expandedRows.includes(comment.id);
                                                        const isLongComment = comment.comment && comment.comment.length > 100;
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                {/* 첫 번째 줄: 메타데이터 및 액션 버튼 */}
                                                                <Box sx={{ 
                                                                    display: 'flex', 
                                                                    alignItems: 'center',
                                                                    justifyContent: 'space-between', 
                                                                    p: 2,
                                                                    bgcolor: comment.id % 2 === 0 ? '#fafafa' : 'white',
                                                                    ...
                                                                    <Box sx={{ 
                                                                        ...
                                                                        <Box sx={{ 
                                                                            ...
                                                                            alignItems: 'center', 
                                                                            gap: 0.5,
                                                                            color: comment.approval_yn === 'Y' ? green[700] : red[700],
                                                                            fontWeight: 500,
                                                                            fontSize: '13px',
                                                                            width: { xs: '50%', sm: '80px' }
                                                                        }}>
                                                                            {comment.approval_yn === 'Y' ? (
                                                                                <>
                                                                                    <CheckCircleIcon sx={{ fontSize: 16 }} />
                                                                                    <span>승인</span>
                                                                                </>
                                                                    ...
                                                                    <Box sx={{ display: 'flex', gap: 1, flexShrink: 0 }}>
                                                                        {comment.approval_yn === 'Y' ? (
                                                                            <Button
                                                                                ...
                                                                                color="error"
                                                                                size="small"
                                                                                onClick={() => handleCommentApproval(comment.comment_no, false)}
                                                                                sx={{ 
                                                                                    fontSize: '11px',
                                                                                    height: '26px',
                                                                            ...
                                                                            <Button
                                                                                ...
                                                                                color="success"
                                                                                size="small"
                                                                                onClick={() => handleCommentApproval(comment.comment_no, true)}
                                                                                sx={{ 
                                                                                    fontSize: '11px',
                                                                                    height: '26px',
                                                                                    minWidth: { xs: '40px', sm: '60px' }
                                                                ...
                                                                <Box sx={{ 
                                                                    ...
                                                                    <Typography sx={{ fontSize: '12px', color: 'text.secondary' }}>
                                                                        등록일: {comment.reg_date ? new Date(comment.reg_date).toLocaleString('ko-KR') : ''}
                                                                    </Typography>
                                                                </Box>
                                                            </Paper>
                                                        );
                                        ...
                                        return (
                                            <Box sx={{ 
                                                ...
                                                <Typography>
                                                    {commentFilter === 'approved' ? '승인된 댓글이 없습니다.' : 
                                                     commentFilter === 'unapproved' ? '미승인 댓글이 없습니다.' : 
                                                     '댓글이 없습니다.'}
                                                </Typography>
                                            </Box>
                                        );
